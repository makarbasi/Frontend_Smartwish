...
export async function GET(request: NextRequest) {
  try {
    ...
    const categoriesResponse = await fetch('https://smartwish.onrender.com/templates-enhanced/categories', {
      headers: {
    ...
    const categoryTemplates = await Promise.all(
      categories.map(async (category: any) => {
        try {
          const apiUrl = new URL('https://smartwish.onrender.com/templates-enhanced/templates')
